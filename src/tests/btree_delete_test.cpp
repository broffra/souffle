...
TEST(BTreeDelete, Basic) {
   const bool DEBUG = false;

   using test_set = btree_delete_set<int, detail::comparator<int>, std::allocator<int>, 16>;
...
TEST(BTreeDelete, Copy) {
   using test_set = btree_delete_set<int>;
...
TEST(BTreeDelete, IteratorEmpty) {
   using test_set = btree_delete_set<int, detail::comparator<int>, std::allocator<int>, 16>;
   test_set t;
...
TEST(BTreeDelete, IteratorBasic) {
   using test_set = btree_delete_set<int, detail::comparator<int>, std::allocator<int>, 16>;
   test_set t;
...
TEST(BTreeDelete, IteratorStress) {
   using test_set = btree_delete_set<int, detail::comparator<int>, std::allocator<int>, 16>;
...
TEST(BTreeDelete, BoundaryEmpty) {
   using test_set = btree_delete_set<int, detail::comparator<int>, std::allocator<int>, 16>;
...
TEST(BTreeDelete, ChunkSplitStress) {
   using test_set = btree_delete_set<int, detail::comparator<int>, std::allocator<int>, 16>;
...
TEST(Performance, Basic) {
   ...
   using t2 = btree_delete_set<Entry, detail::comparator<Entry>, std::allocator<Entry>, 256, detail::linear_search>;
   checkPerformance(t2, "souffle btree_delete_set - 256 - linear", in, out);

   using t3 = btree_delete_set<Entry, detail::comparator<Entry>, std::allocator<Entry>, 256, detail::binary_search>;
   checkPerformance(t3, "souffle btree_delete_set - 256 - binary", in, out);
}
...
TEST(Performance, Load) {
   ...
   time("conventional load", [&]() { btree_delete_set<int> t(data.begin(), data.end()); });

   // take time for structured load
   time("bulk-load", [&]() { auto t = btree_delete_set<int>::load(data.begin(), data.end()); });
}

TEST(BTreeDelete, Parallel) {
   ...
   btree_delete_set<entry_t> filter;
...
TEST(BTreeDelete, ParallelScaling) {
   using test_set = btree_delete_set<int>;
